...
class HttpClientMockBuilderTest {
    ...
    void shouldMatchSeparateHostAndPath() throws Exception {
        ...
        assertThrows(NoMatchingRuleException.class, () -> httpClientMock.send(TestRequests.post("http://www.google.com/login"), discarding()));
        assertThrows(NoMatchingRuleException.class, () -> httpClientMock.send(TestRequests.post("http://localhost/foo"), discarding()));
        assertThat(httpClientMock.send(TestRequests.post("http://localhost/login"), discarding()).statusCode(), equalTo(200));
    }
    ...
    void shouldMatchSeparatePathAndParameter() throws Exception {
        ...
        assertThrows(NoMatchingRuleException.class, () -> httpClientMock.send(TestRequests.post("http://localhost/login?a=1&b=2"), ofString()));

        assertThat(firstResponse, HttpResponseMatchers.hasContent("one"));
        assertThat(secondResponse, HttpResponseMatchers.hasContent("two"));
    }
    ...
    void onXXXX_methods_with_path_should_add_method_and_path_condition() throws Exception {
        ...
        assertThat(getResponse, HttpResponseMatchers.hasContent("get"));
        assertThat(postResponse, HttpResponseMatchers.hasContent("post"));
        assertThat(putResponse, HttpResponseMatchers.hasContent("put"));
        assertThat(deleteResponse, HttpResponseMatchers.hasContent("delete"));
        assertThat(headResponse, HttpResponseMatchers.hasContent("head"));
        assertThat(optionsResponse, HttpResponseMatchers.hasContent("options"));
        assertThat(patchResponse, HttpResponseMatchers.hasContent("patch"));
    }
    ...
    void shouldCheckCustomRule() throws Exception {
        ...
        assertThat(first, HttpResponseMatchers.hasContent("yes"));
    }
    ...
    void shouldUseRightHostAndPath() throws Exception {
        ...
        final HttpResponse google = httpClientMock.send(TestRequests.get("http://www.google.com"), ofString());
        final HttpResponse https = httpClientMock.send(newBuilder(URI.create("https://www.google.com")).GET().build(), ofString());

        assertThat(localhost, HttpResponseMatchers.hasContent("localhost"));
        assertThat(google, HttpResponseMatchers.hasContent("google"));
        assertThat(https, HttpResponseMatchers.hasContent("https"));
    }
    ...
    void shouldMatchRightHeaderValue() throws Exception {
        ...
        assertThat(getMozilla, HttpResponseMatchers.hasContent("mozilla"));
        assertThat(getChrome, HttpResponseMatchers.hasContent("chrome"));
    ...
    void should_match_right_parameter_value() throws Exception {
        ...
        assertThat(one, HttpResponseMatchers.hasContent("one"));
        assertThat(two, HttpResponseMatchers.hasContent("two"));
    }
    ...
    void should_add_default_host_to_every_relative_path() throws Exception {
        ...
        assertThat(login, HttpResponseMatchers.hasContent("login"));
        assertThat(search, HttpResponseMatchers.hasContent("search"));
        assertThat(logout, HttpResponseMatchers.hasContent("logout"));
    }
    ...
    void checkBody() throws Exception {
        ...
        assertThat(correctLogin, HttpResponseMatchers.hasStatus(200));
        assertThat(badLogin, HttpResponseMatchers.hasStatus(500));
    }
...
//        HttpResponse notFound_2 = httpClientMock.execute(new HttpPost("http://localhost/login?user=john&pass=abc&foo=bar"));
        ...
        final var wrong = httpClientMock.send(TestRequests.post("http://localhost/login?user=john"), discarding());
        final var ok = httpClientMock.send(TestRequests.post("http://localhost/login?user=john&pass=abc"), discarding());

        //assertThat(notFound, hasStatus(404));
        assertThat(wrong, HttpResponseMatchers.hasStatus(400));
        assertThat(ok, HttpResponseMatchers.hasStatus(200));
    ...
    void should_handle_path_with_parameters_and_reference() throws Exception {
        ...
        assertThrows(NoMatchingRuleException.class, () -> httpClientMock.send(TestRequests.post("http://localhost/login"), discarding()));
        assertThrows(NoMatchingRuleException.class, () -> httpClientMock.send(TestRequests.post("http://localhost/login?p=1"), discarding()));
        assertThrows(NoMatchingRuleException.class, () -> httpClientMock.send(TestRequests.post("http://localhost/login#abc"), discarding()));
    ...
    void after_reset_number_of_calls_should_be_zero() throws Exception {
        ...
        httpClientMock.onPost("/login").doReturnStatus(200);
        httpClientMock.send(TestRequests.post("http://localhost/login"), discarding());
        httpClientMock.send(TestRequests.post("http://localhost/login"), discarding());
        httpClientMock.reset();
    ...
    void not_all_parameters_occurred() {
        ...
        assertThrows(NoMatchingRuleException.class, () -> httpClientMock.send(TestRequests.post("http://localhost/login"), ofString()));
    }
    ...
    void should_allow_different_host_then_default() throws Exception {
        ...
        final var login = httpClientMock.send(TestRequests.get("http://localhost/login"), ofString());
        final var google = httpClientMock.send(TestRequests.get("http://www.google.com"), ofString());
    ...
    void response_should_contain_request_headers_body_uri_version() throws IOException, URISyntaxException {
        ...
        httpClientMock.onGet("/login")
                .doReturn("login")
                .withHeader("foo", "bar");
        var request = TestRequests.get("http://localhost/login");
